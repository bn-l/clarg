...
// ============================================================================
// special_flags: no_system_dirs / no_root wiring
// ============================================================================

#[test]
fn test_grep_no_system_dirs_blocks_usr() {
    let tmp = TempDir::new().unwrap();
    let config = Config {
        block_access_to: vec![],
        commands_forbidden: vec![],
    ...
    let ruleset = RuleSet::build(&config, tmp.path()).unwrap();
    let input = make_grep_input_path_only("/usr/share", tmp.path().to_path_buf());
...
#[test]
fn test_grep_no_root_blocks_root() {
    let tmp = TempDir::new().unwrap();
    let config = Config {
        block_access_to: vec![],
        commands_forbidden: vec![],
    ...
    let ruleset = RuleSet::build(&config, tmp.path()).unwrap();
    let input = make_grep_input_path_only("/", tmp.path().to_path_buf());
...
// ============================================================================
// Regression: ~user on Grep must be resolved, not joined to project.
// ============================================================================

#[test]
fn test_grep_tilde_root_blocked_by_no_system_dirs() {
    let tmp = TempDir::new().unwrap();
    let config = Config {
        block_access_to: vec![],
        commands_forbidden: vec![],
    ...
    let ruleset = RuleSet::build(&config, tmp.path()).unwrap();
    let input = make_grep_input_path_only("~root", tmp.path().to_path_buf());
    match ruleset.evaluate(&input) {
        Verdict::Allow => panic!("expected deny — ~root bypass on Grep"),
        Verdict::Deny(reason) => {
            assert!(reason.contains("no_system_dirs"), "got: {}", reason);
        }
    }
